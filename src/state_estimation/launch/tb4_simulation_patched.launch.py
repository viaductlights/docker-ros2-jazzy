...
def generate_launch_description():
    ...
    # ros2-gz bridge config patched for filter assignment
    ...
    bridge_config = LaunchConfiguration('bridge_config') #ros-gz bridge patch
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(my_package, 'config', 'nav2_params.yaml'), # patched
    ...
    declare_rviz_config_file_cmd = DeclareLaunchArgument(
        'rviz_config_file',
        default_value=os.path.join(my_package, 'config', 'error.rviz'), # patched for project
    ...
    #patched cmd_vel type Twist to TwistStamped
    declare_bridge_config_cmd = DeclareLaunchArgument(
        'bridge_config',
        default_value=os.path.join(my_package, 'config', 'tb4_bridge_patch.yaml'),
        description='ros-gz bridge config patch',
    )
    ...
    gz_robot = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(my_package, 'launch', 'spawn_tb4_patched.launch.py')), # patched
        launch_arguments={'namespace': namespace,
                          'use_simulator': use_simulator,
                          'use_sim_time': use_sim_time,
                          'bridge_config': bridge_config, # patched
    ...
    ld.add_action(declare_bridge_config_cmd) # patched
