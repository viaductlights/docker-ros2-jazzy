...
private:
    ...
    void sendNavGoal()
    {
    	...
    	if (target_poses_.empty()) {
        	...
        	target_poses_.push_back(make_pose(4.0, 0.0, 0.0, 1.0));  // waypoint 2
        	target_poses_.push_back(make_pose(7.0, 6.0, 0.0, 1.0));  // waypoint 3
        	target_poses_.push_back(make_pose(7.0, 2.35, 0.0, 0.0)); // waypoint 4
        	current_pose_idx_ = 0;
    	}
