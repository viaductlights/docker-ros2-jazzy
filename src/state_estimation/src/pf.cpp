...
public:
    ParticleFilter() : Node("pf"), num_particles_(500) {
 ...
 scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
  ...
  private:
	  ...
	  double prev_yaw = extractYaw(last_odom.pose.pose.orientation);
          ...
          // Convert to delta in robot's local frame or absolute 
          // For brief barebones: standard odometry motion model noise addition
          std::normal_distribution<double> noise_motion(0.0, 0.05);
	  ...
	  last_odom = *msg;
    	...
    	void scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
          ...
          publishEndpoints({ScanPoints_}, "base_link", msg->header.stamp);  // debug viz
...
void update() {
 ...
 void extract_scan_(const sensor_msgs::msg::LaserScan& scan) {
	...
	void publishEndpoints(const std::vector<std::vector<Eigen::Vector2d>>& endpoints,
        ...
        endpoint_pub_->publish(marker_array);
    }
	...
	rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr endpoint_pub_;
